...
IMAGE img_bk; // 背景图片
float position_x,position_y; // 仙人掌位置
float bullet_x,bullet_y; // 子弹位置
float enemy_x,enemy_y; // 气球位置
IMAGE img_planeNormal1,img_planeNormal2; // 正常仙人掌图片
IMAGE img_planeExplode1,img_planeExplode2; // 哭泣仙人掌图片
IMAGE img_bullet1,img_bullet2; // 子弹图片
IMAGE img_enemyPlane1,img_enemyPlane2; // 气球图片
int isExpolde = 0; // 仙人掌是否哭泣
int score = 0; // 得分
...
void show()
{
	...
	if (isExpolde==0)
	{
		putimage(position_x-50, position_y-60, &img_planeNormal1,NOTSRCERASE); // 显示正常仙人掌	
		...
		putimage(enemy_x, enemy_y, &img_enemyPlane1,NOTSRCERASE); // 显示气球	
		putimage(enemy_x, enemy_y, &img_enemyPlane2,SRCINVERT);
	}
	else
	{
		putimage(position_x-50, position_y-60, &img_planeExplode1,NOTSRCERASE); // 显示哭泣仙人掌	
...
void updateWithoutInput()
{
	...
	{		
		...
		if (enemy_y<High-25)
			...
			enemy_y = 10;		
		if (abs(bullet_x-enemy_x)+abs(bullet_y-enemy_y) < 80)  // 子弹击中气球
		{
			enemy_x = rand()%Width;
			enemy_y = -40;
			bullet_y = -85;	
